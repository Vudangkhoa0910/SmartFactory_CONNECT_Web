...
import { Room, RoomBooking, MeetingPurpose } from '../../../types/room-booking.types';
import { Modal } from '../../ui/modal';
import { useTranslation } from "../../../contexts/LanguageContext";
import { TIME_SLOTS, MEETING_PURPOSE_OPTIONS } from './constants';
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-[700px] p-6 lg:p-10">
      <div className="flex flex-col px-2 overflow-y-auto custom-scrollbar max-h-[80vh]">
        ...
        <div className="mt-8 space-y-6">
            ...
            >
              ...
              {rooms.map((room) => (
                <option key={room.id} value={room.id}>
                  {room.name} ({room.capacity} {t('booking.info.attendees_unit')})
                </option>
              ))}
            </select>
          </FormField>
          ...
          {/* Meeting Purpose */}
          <FormField label={t('booking.info.meeting_purpose')}>
            <select
          ...
          {/* Expected Attendees */}
          <FormField label={t('booking.info.expected_attendees')} required>
            <input
              type="number"
              min="1"
