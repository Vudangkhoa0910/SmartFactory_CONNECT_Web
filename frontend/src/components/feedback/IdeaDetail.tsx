...
export const IdeaDetail: React.FC<IdeaDetailProps> = ({
  ...
  const handleSendSolution = () => {
    ...
    setSolutionInput("");
    onUpdateStatus(t('idea.solution_proposal'), newSolution.note, newSolution.status);
  };
  ...
  const handleApprove = () => {
    ...
    onUpdateStatus(t('status.approved'), note, t('status.approved'));
  };
  ...
  const handleReject = () => {
    ...
    setSolution(null);
    setSolutionInput("");
    onUpdateStatus(t('status.rejected'), note, t('button.reject'));
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-white dark:bg-neutral-900 transition-colors">
      ...
      <div key={idea.id} className="overflow-y-auto flex-1 bg-gray-50 dark:bg-neutral-900">
