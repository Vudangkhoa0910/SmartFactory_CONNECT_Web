 ...
 * - Ideas: Supervisor -> Manager -> GM
 */

const db = require('../config/database');

// Handler level definitions
const INCIDENT_LEVELS = {
  1: { name: 'Team Leader', role: 'team_leader' },
  2: { name: 'Supervisor', role: 'supervisor' },
  3: { name: 'Manager', role: 'production_manager' },
  4: { name: 'Admin', role: 'admin' },
};

const IDEA_LEVELS = {
  1: { name: 'Supervisor', role: 'supervisor' },
  2: { name: 'Manager', role: 'production_manager' },
  3: { name: 'General Manager', role: 'factory_manager' },
};
...
const escalateIncident = async (incidentId, escalatedBy, reason, isAutomatic = false) => {
  ...
  try {
    ...
    return {
      ...
      levelName: INCIDENT_LEVELS[nextLevel]?.name
    };
...
const escalateIdea = async (ideaId, escalatedBy, reason, isAutomatic = false) => {
  ...
  try {
    ...
    return {
      ...
      newLevel: nextLevel,
      newHandler: nextHandler,
      levelName: IDEA_LEVELS[nextLevel]?.name
    };
...
const findNextHandler = async (client, entityType, level, departmentId) => {
  const levels = entityType === 'incident' ? INCIDENT_LEVELS : IDEA_LEVELS;
