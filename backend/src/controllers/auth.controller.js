...
const register = asyncHandler(async (req, res) => {
  ...
  // Validate role
  if (!isValidRole(role)) {
    throw new AppError(`Invalid role: ${role}. Valid roles are: ${ALL_ROLES.join(', ')}`, 400);
  }
  ...
  // Get level from centralized constants
  const level = getLevelFromRole(role);
...
const login = asyncHandler(async (req, res) => {
  ...
  const userHasWebAccess = hasWebAccess(user.role);
  const userCanManageUsers = canManageUsers(user.role);
  const userCanViewPinkBox = canViewPinkBox(user.role);
  const userCanReviewIdeas = canReviewIdeas(user.role);
  const userCanCreateNews = canCreateNews(user.role);
  const userCanApproveBookings = canApproveRoomBookings(user.role);
  const userCanAssignIncidents = canAssignIncidents(user.role);
  ...
  res.json({
    ...
    data: {
      ...
      user: {
        ...
        permissions: {
          has_web_access: userHasWebAccess,
          has_mobile_access: true, // All users can use mobile
          can_view_dashboard: userHasWebAccess,
          can_manage_users: userCanManageUsers,
          can_review_ideas: userCanReviewIdeas,
          can_view_pink_box: userCanViewPinkBox,
          can_create_news: userCanCreateNews,
          can_approve_bookings: userCanApproveBookings,
          can_assign_incidents: userCanAssignIncidents
        }
      }
    }
  });
 ...
 * For App Mobile - upload avatar with multipart/form-data
 */
const uploadAvatar = asyncHandler(async (req, res) => {
  ...
  const { uploadFilesToGridFS } = require('../utils/media-upload.helper');

  // Upload to MongoDB GridFS
  const attachments = await uploadFilesToGridFS(req.files, {
    type: 'avatar',
    relatedType: 'user',
    relatedId: userId,
    uploadedBy: userId
  ...
  const result = await db.query(`
    ...
    WHERE id = $2
    RETURNING id, employee_code, email, full_name, phone, avatar_url, role, level, department_id
  `, [avatarUrl, userId]);
 ...
 * For App Mobile - change language preference
 */
const updateLanguage = asyncHandler(async (req, res) => {
  const userId = req.user.id;
  const { language } = req.body;

  if (!['vi', 'ja'].includes(language)) {
  ...
  const result = await db.query(`
    ...
    WHERE id = $2
    RETURNING id, employee_code, email, full_name, preferred_language
  `, [language, userId]);
